...
/*   Adds a Nonce to the trace and returns true upon success. 
     Ensuring that the trace invariant is  not violated 
*/     
...
@*/
/*--- Encrypts a message and returns true upon success */
...
/*@ ensures [1/2]ghost_cell<Trace>(snapId, snapshot) 
 ...
 /*--- Decrypts a message and returns true upon success */ 
...
/*@ lemma void wasDecryptedTrue(trace<EventParams> snapshot, Term ciphertextT, Term recv_skT, Term msgT, int owner)
requires ciphertextT == encrypt(createPk(recv_skT), msgT) &*& canDecrypt(snapshot, ciphertextT, recv_skT, owner) == true;
ensures wasDecrypted(snapshot, msgT, recv_skT, owner) == true;
{
